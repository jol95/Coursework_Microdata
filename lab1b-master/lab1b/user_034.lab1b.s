...
# ----------------------------------------------------------
	...
	.set noreorder
main:	
	li	$s0, 0x3179c241	# initialize "seed"
	# li	$s0, 0x6e7af858	# initialize "seed" TESTTESTTEST
	la	$s1, seed	# initialize "seed"
	sw	$s0, 0($s1)
	la	$a0, coded	# address of start of coded words REAL
	# la	$a0, abc	# address of start of coded words ABC
...
exit1:
	move  $sp, $fp		# all local var's gone: easy !
